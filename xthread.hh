...
public:
  ...
  void initialize() {
    ...
    _xmap.initialize(HashFuncs::hashAddr, HashFuncs::compareAddr,
                     THREAD_MAP_SIZE);
  ...
  // This function is only called in the current thread before the real thread
  // function
  ...
  int thread_create(pthread_t *tid, const pthread_attr_t *attr,
    ...
    pthread_attr_setstack(&iattr,
                          (void *)(globalStackAddr +
                                   (intptr_t)tindex * STACK_SIZE +
                                   GUARD_PAGE_SIZE),
                          STACK_SIZE - 2 * GUARD_PAGE_SIZE);
    /* Guard pages have already been setted before main()
    if(0 != mprotect((void*)(childrenStackStart + STACK_SIZE - GUARD_PAGE_SIZE),
    GUARD_PAGE_SIZE, PROT_NONE)
                    || 0 != mprotect((void*)childrenStackStart, GUARD_PAGE_SIZE,
    PROT_NONE)) { fprintf(stderr, "Failed to set guard page for thread#%d\n",
    tindex); abort();
    }
    */

    int result = Real::pthread_create(tid, &iattr, xthread::startThread,
                                      (void *)children);
#else
    int result =
        Real::pthread_create(tid, attr, xthread::startThread, (void *)children);
#endif
  ...
  void acquireGlobalLock() { spin_lock(); }

  void releaseGlobalLock() { spin_unlock(); }

private:
  inline void spin_lock() { pthread_spin_lock(&_spin_lock); }

  inline void spin_unlock() { pthread_spin_unlock(&_spin_lock); }
  ...
  threadHashMap
      _xmap; // The hash map that map the address of pthread_t to thread index.
