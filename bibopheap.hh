...
public:
  ...
  void *initialize() {
    ...
    unsigned randPower = getRandomNumber() %
                         (LOG2(MAX_RANDOM_BAG_SIZE / MIN_RANDOM_BAG_SIZE) + 1);
    ...
    PRINF("_bibopBagSize=0x%lx, _bagShiftBits=%ld, sizeof(PerThreadBag)=%zu, "
          "_shadowObjectInfoSize=%zu",
          _bibopBagSize, _bagShiftBits, sizeof(PerThreadBag),
          _shadowObjectInfoSize);
    PRINF("BIBOP_NUM_BAGS=%u, _numUsableBags=%u", BIBOP_NUM_BAGS,
          _numUsableBags);
    ...
    size_t unusableHeapSpace =
        (BIBOP_NUM_BAGS - _numUsableBags) * _bibopBagSize;
    ...
    for (threadNum = 0; threadNum < MAX_ALIVE_THREADS; threadNum++) {
      ...
      for (bagNum = 0; bagNum < _numUsableBags; bagNum++) {
        ...
        for (int curBagSetItem = 0; curBagSetItem < BIBOP_BAG_SET_SIZE;
             curBagSetItem++) {
        ...
        if (bagNum == _lastUsableBag) {
          // If this bag can only fit one object, forego the use of a guard
          // object.
          if (_bibopBagSize == lastUsableBagSize) {
            guardoffset = 0;
            guardsize = 0;
          }
          guardsize += (BIBOP_NUM_BAGS - LOG2(lastUsableBagSize) +
                        LOG2(BIBOP_MIN_BLOCK_SIZE) - 1) *
                       _bibopBagSize;

          // PRDBG("last usable bag: lastUsableBagSize=%zu, _bibopBagSize=%zu,
          // guardsize=%zu, guardoffset=%zu", 		lastUsableBagSize,
          //_bibopBagSize, guardsize, guardoffset);
        }
        ...
        curBag->nextHeapObjectOffset =
            BIBOP_HEAP_SIZE * (BIBOP_BAG_SET_SIZE - 1) +
            (BIBOP_HEAP_SIZE - _bibopBagSize + classSize + guardoffset);
        for (int curBagSetItem = 0; curBagSetItem < BIBOP_BAG_SET_SIZE;
             curBagSetItem++) {
          // Initialize bump pointer to the first object
          curBag->position[curBagSetItem] =
              _heapBegin + offsetBag + (curBagSetItem * BIBOP_HEAP_SIZE);
          curBag->lastofCurBag[curBagSetItem] = getLastOfBag(
              curBag->position[curBagSetItem], guardoffset, classSize);
// ptrdiff_t diff = curBag->lastofCurBag[curBagSetItem] -
// curBag->position[curBagSetItem];  PRINF("thread %u bag %u set %d:
// classSize=%zu, guardsize=%zu, guardoffset=%zu, lastofCurBag=%p, position=%p,
// diff=%lu",
//				threadNum, bagNum, curBagSetItem, classSize,
//curBag->guardsize, guardoffset, curBag->lastofCurBag[curBagSetItem],
//curBag->position[curBagSetItem], diff);
#ifdef ENABLE_GUARDPAGE
          setGuardPage(curBag->position[curBagSetItem], curBag->guardsize,
                       guardoffset);
#endif
        }
        ...
        curBag->cflthreshold =
            numBagObjects / CACHEDFREELIST_THRESHOLD_RATIO_BYBAG;
    ...
    _shadowMemSizePerHeapCeilShiftBits =
        (sizeof(size_t) * 8) - __builtin_clzl(_shadowMemSizePerHeap - 1);
    ...
    for (threadNum = 0; threadNum < MAX_ALIVE_THREADS; threadNum++) {
      for (bagNum = 0; bagNum < _numUsableBags; bagNum++) {
        PerThreadBag *curBag = &_threadBag[threadNum][bagNum];
        curBag->nextShadowHeapObjectOffset =
            _shadowMemSizePerHeapCeil * BIBOP_BAG_SET_SIZE -
            ((unsigned long)(curBag->numObjects - 1)
             << _shadowObjectInfoSizeShiftBits);
      }
    }

    allocShadowMem();
    PRINF("_shadowMemBegin=%p, _shadowMemEnd=%p, _shadowMemSizePerHeap=%zu, "
          "_smSPHeapCeilShiftBits=%u",
          _shadowMemBegin, _shadowMemEnd, _shadowMemSizePerHeap,
          _shadowMemSizePerHeapCeilShiftBits);
...
//  bag->threadIndex, bag->bagNum, numBagSetItem, addr, addrEnd);
  ...
  void *allocateSmallObject(size_t sz) {
    ...
    if (!IS_FREELIST_EMPTY(&curBag->freelist[numBagSetItem]) &&
        !useBumpPointer) {
      ...
      shadowinfo =
          (shadowObjectInfo *)FREELIST_REMOVE(&curBag->freelist[numBagSetItem]);
      ...
      // Save the current value of the position pointer, as this will be used to
      // allocate the object requested by the caller. The position pointer will
      // then be modified to point to the next available object.
      ptr = *position;
    ...
    // PRDBG("thread %u bag %u set %u malloc size %zu(%zu) @ %p ~ %p (canary @
    // %p, sm @ %p)", 	curBag->threadIndex, curBag->bagNum, numBagSetItem, sz,
    //curBag->classSize, 	ptr, ptrEnd, canary_dbg, shadowinfo);
  ...
  inline void incrementBumpPointer(PerThreadBag *curBag,
                                   unsigned numBagSetItem) {
    char **position = &curBag->position[numBagSetItem];
    ...
    //				curBag->threadIndex, curBag->bagNum, numBagSetItem, heapNum,
    //*position, 				*lastofCurBag);
    ...
    if (*position < *lastofCurBag) {
      ...
      *lastofCurBag = getLastOfBag(*position, guardoffset, curBag->classSize);
      // unsigned heapNum = getHeapNumber(*position);
      // PRDBG("thread %u bag %u set %u: moved to heap number %u: bag start=%p,
      // lastofCurBag=%p", 				curBag->threadIndex,
      //curBag->bagNum, numBagSetItem, heapNum, oldValue, *lastofCurBag);
    }
  }
  ...
  bool tryRandomGuardPage(PerThreadBag *curBag, unsigned numBagSetItem) {
    ...
    if (getRandomNumber() < RANDOM_GUARD_RAND_CUTOFF) {
      ...
      if(mprotect(savedPosition, guardSize, PROT_NONE) == -1) {
                      PRERR("mprotect(%p, %zu, PROT_NONE) failed: %s",
      savedPosition, guardSize, strerror(errno));
      }
      */
  ...
  void freeSmallObject(void *addr) {
    unsigned numBagSetItem;
    PerThreadBag *bag;
    shadowObjectInfo *shadowinfo =
        getShadowObjectInfo(addr, &bag, &numBagSetItem);
    ...
    if (isObjectFree(shadowinfo)) {
      PRERR("Double free or invalid free problem found on object %p (sm %p)",
            addr, shadowinfo);
      printCallStack();
      exit(EXIT_FAILURE);
    } else {
      ...
      for (int move = LEFT; move <= RIGHT; move++) {
        ...
        for (int pos = 0; pos < NUM_MORE_CANARIES_TO_CHECK; pos++) {
          if ((canaryShadow =
                   getNextCanaryNeighbor(canaryShadow, bag, (direction)move))) {
            char *neighborAddr =
                (char *)getAddrFromShadowInfo(canaryShadow, bag);
            char *canary = neighborAddr + bag->classSize - 1;
            // We will only inspect the canary of objects currently in-use; if
            // the object is free, then it has already been checked previously.
            if (!isObjectFree(canaryShadow) && (*canary != CANARY_SENTINEL)) {
              FATAL("canary value for object %p (neighbor of %p) not intact; "
                    "canary @ %p, value=0x%x",
                    neighborAddr, addr, canary, *canary);
            }
          } else {
            // getNextCanaryNeighbor will only return null when we attempt to
            // move left from the first object in a bag within a heap that is
            // less than BIBOP_BAG_SET_SIZE. This indicates there are no
            // previous heaps we can move to. In this case, simply stop trying
            // to move left.
            break;
          }
        }
...
private:
  inline void lock(PerThreadBag *bag, unsigned numBagSetItem) {
    pthread_spin_lock(&bag->listlock[numBagSetItem]);
  }
  inline void unlock(PerThreadBag *bag, unsigned numBagSetItem) {
    pthread_spin_unlock(&bag->listlock[numBagSetItem]);
  }
...
#warning using libc random number generator
  ...
  inline shadowObjectInfo *getNextCanaryNeighbor(shadowObjectInfo *shadowinfo,
                                                 PerThreadBag *bag,
                                                 direction move) {
    unsigned heapNum = getHeapNumber(shadowinfo);
    ptrdiff_t shadowOffset = (char *)shadowinfo - _shadowMemBegin;
    unsigned long objectindex = ((shadowOffset & _shadowMemSizePerHeapMask) -
                                 bag->startShadowMemOffset) >>
                                _shadowObjectInfoSizeShiftBits;
    ...
    if (move == LEFT) {
      if (objectindex == 0) {
        // Check whether we are physically capable of moving to the left; if
        // not, return null
        if (heapNum < BIBOP_BAG_SET_SIZE) {
          // PRDBG("thread %u bag %u: no more canary neighbors to check; sm %p
          // in heap %u", 				bag->threadIndex,
          //bag->bagNum, shadowinfo, heapNum);
          return NULL;
        }

        // We must move to the last object in the previous heap
        // shadowObjectInfo * shadowinfoOld = shadowinfo;
        shadowinfo = (shadowObjectInfo *)((char *)shadowinfo -
                                          bag->nextShadowHeapObjectOffset);
        // PRDBG("thread %u bag %u: canary neighbor moving to prev heap sm %p ->
        // sm %p, offset=%zu",
        //				bag->threadIndex, bag->bagNum, shadowinfoOld,
        //shadowinfo, bag->nextShadowHeapObjectOffset);
      } else {
        shadowinfo--;
      }
    } else {
      ...
      if (objectindex == bag->lastObjectIndex) {
        ...
        shadowinfo = (shadowObjectInfo *)((char *)shadowinfo +
                                          bag->nextShadowHeapObjectOffset);
        // PRDBG("thread %u bag %u: canary neighbor moving to next heap sm %p ->
        // sm %p, offset=%zu",
        //				bag->threadIndex, bag->bagNum, shadowinfoOld,
        //shadowinfo, bag->nextShadowHeapObjectOffset);
      } else {
  ...
  inline shadowObjectInfo *getShadowObjectInfo(void *addr, PerThreadBag **bag,
                                               unsigned *numBagSetItem = NULL) {
    ...
    // PRDBG("addr=%p, _heapBegin=%p, offset=0x%lx, localHeapOffset=0x%lx,
    // localBagOffset=0x%lx, globalBagNum=%lu, heapIndex=%lu, _heapMask=0x%lx,
    // _bagMask=0x%lx, _numBagsPerHeapShiftBits=%u, _numBagsPerSubHeapMask=0x%lx,
    // bag num result=%lu", 	addr, _heapBegin, offset, localHeapOffset,
    //localBagOffset, globalBagNum, heapIndex, _heapMask, _bagMask,
    //_numBagsPerHeapShiftBits, _numBagsPerSubHeapMask, (globalBagNum &
    //_numBagsPerSubHeapMask));

    // Now we will locate the PerThreadBag based on the bag number and heap
    // offset.
    *bag = &_threadBag[localHeapOffset >> _threadShiftBits]
                      [globalBagNum & _numBagsPerSubHeapMask];
    ...
    if ((localBagOffset & (*bag)->classMask) != 0) {
      PRERR("Invalid object: addr %p, classSize 0x%lx, classMask 0x%lx, offset "
            "0x%lx",
    ...
    shadowObjectInfo *shadowinfo =
        (shadowObjectInfo *)(_shadowMemBegin +
                             (heapIndex << _shadowMemSizePerHeapCeilShiftBits) +
                             (*bag)->startShadowMemOffset);
  ...
  inline shadowObjectInfo *getShadowObjectInfo(void *addr, PerThreadBag *bag,
    ...
    shadowObjectInfo *bagShadowInfo =
        (shadowObjectInfo *)(_shadowMemBegin +
                             (heapIndex << _shadowMemSizePerHeapCeilShiftBits) +
                             bag->startShadowMemOffset);
    ...
    if (!debug) {
      ...
      if (checkAddr != addr) {
        ...
        PRERR("addr=%p, offset into heap=%lu, localBagOffset=%lu, "
              "globalBagNum=%lu, heapIndex=%lu, bagShadowInfo=%p, retval=%p",
              addr, offset, localBagOffset, globalBagNum, heapIndex,
              bagShadowInfo, retval);
        ...
        for (int i = 0; i < BIBOP_BAG_SET_SIZE; i++) {
          PRERR("\t lastofCurBag[%d]=%p", i, bag->lastofCurBag[i]);
        }
        raise(SIGUSR2);
      }
    }
  ...
  inline void *getAddrFromShadowInfo(shadowObjectInfo *shadowaddr,
                                     PerThreadBag *bag, bool debug = false) {
    ...
    objectindex = ((shadowOffset & _shadowMemSizePerHeapMask) -
                   bag->startShadowMemOffset) >>
                  _shadowObjectInfoSizeShiftBits;
...
#ifdef DEBUG
    if (!debug) {
      void *retval = (void *)(_heapBegin + heapOffset + bag->startOffset +
                              (objectindex << bag->shiftBits));
      ...
      if (checkAddr != shadowaddr) {
        ...
        PRERR("shadowaddr=%p, shadowOffset=%lu, heapIndex=%lu, "
              "objectindex=%lu, heapOffset=%lu, retval=%p",
              shadowaddr, shadowOffset, heapIndex, objectindex, heapOffset,
              retval);
    ...
    return retval;
#else
    return (void *)(_heapBegin + heapOffset + bag->startOffset +
                    (objectindex << bag->shiftBits));
#endif
  }
...
#ifdef ENABLE_GUARDPAGE
  inline int setGuardPage(void *bagStartAddr, size_t guardsize,
                          size_t guardoffset) {
    if (guardsize == 0) {
      return 0;
    }
    ...
    // PRDBG("setGuardPage: mprotect region 0x%lx ~ 0x%lx, size=%lu", guardaddr,
    // endAddr, diff);
