...
#ifdef LOGTOFILE
inline int getOutputFD() { return outputfd; }
#endif

#ifndef CUSTOMIZED_STACK
inline int getThreadIndex() { return _threadIndex; }

inline void setThreadIndex(int index) { _threadIndex = index; }
#endif
inline size_t alignup(size_t size, size_t alignto) {
  return (size % alignto == 0) ? size
                               : ((size + (alignto - 1)) & ~(alignto - 1));
}

inline void *alignupPointer(void *ptr, size_t alignto) {
  return ((intptr_t)ptr % alignto == 0)
             ? ptr
             : (void *)(((intptr_t)ptr + (alignto - 1)) & ~(alignto - 1));
}
...
#define MAX_RANDOM_BAG_SIZE 0x1000000 // 16MB
   //#define BIBOP_BAG_SIZE 0x100000						//
   //1MB
...
public:
  slist_t listentry; // Always put the next pointer to the first one. Thus, it
                     // is easy to operate for pointer operations
