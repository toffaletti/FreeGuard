...
extern "C" int __libc_start_main(main_fn_t, int, char **, void (*)(),
                                 void (*)(), void (*)(), void *)
    __attribute__((weak, alias("freeguard_libc_start_main")));

extern "C" int freeguard_libc_start_main(main_fn_t main_fn, int argc,
                                         char **argv, void (*init)(),
  ...
  // set guard pages in cusotmized stack area. Set in both the beginnning and
  // end. better way is to set this when we use a new thread index, which may
  // require changing the bool flag in thread_t to a int.
  for (intptr_t i = 1; i < MAX_THREADS; i++) { // ingore the first thread
    intptr_t stackStart = globalStackAddr + i * STACK_SIZE;
    if (0 != mprotect((void *)(stackStart + STACK_SIZE - GUARD_PAGE_SIZE),
  ...
  intptr_t stackTop = (((intptr_t)&main_fn + PageSize) & ~(PageSize - 1)) +
                      PageSize; // page align
  ...
  return real_libc_start_main(main_fn, argc, argv, init, fini, rtld_fini,
                              stack_end);
}
#else
...
void *aligned_alloc(size_t, size_t)
    __attribute__((weak, alias("xxaligned_alloc")));
...
int posix_memalign(void **, size_t, size_t)
    __attribute__((weak, alias("xxposix_memalign")));
}
...
void heapinitialize() {
  if (heapInitStatus == E_HEAP_INIT_NOT) {
    ...
    // The following function will invoke dlopen and will call malloc in the
    // end. Thus, it is putted in the end so that it won't fail
    Real::initializer();
