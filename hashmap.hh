...
public:
  ...
  void initialize(hashFuncPtr hfunc, keycmpFuncPtr kcmp,
    ...
    _entries = (struct HashEntry *)SourceHeap::allocate(
        size * sizeof(struct HashEntry));
  ...
  // If existing, return true. *value should be carried specific value for this
...
private:
  ...
  struct Entry *createNewEntry(const KeyType &key, size_t keylen,
                               ValueType value) {
    struct Entry *entry =
        (struct Entry *)SourceHeap::allocate(sizeof(struct Entry));
  ...
  void insertEntry(struct HashEntry *head, const KeyType &key, size_t keylen,
                   ValueType value) {
    ...
    head, head->count); PRINF("insertEntry entry %p, entrynext %p, at head %p
    hear->list %p headlist->next %p\n", entry, entry->list.next, head,
    &head->list, head->list.next);
        */
  }

  // Search the entry in the corresponding list.
  struct Entry *getEntry(struct HashEntry *first, const KeyType &key,
                         size_t keylen) {
  ...
  iterator begin() {
    ...
    while (pos < _buckets) {
      ...
      if (head->count != 0) {
        ...
        //	PRINF("In begin() function, head %p, firstentry %p, firstentry
