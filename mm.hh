...
#define ALIGN_TO_CACHELINE(size) (size % 64 == 0 ? size : (size + 64) / 64 * 64)
  ...
  static void *mmapAllocateShared(size_t sz, int fd = -1,
                                  void *startaddr = NULL) {
    return allocate(true, sz, fd, startaddr);
  }

  static void *mmapAllocatePrivate(size_t sz, void *startaddr = NULL,
                                   int fd = -1) {
...
private:
  static void *allocate(bool isShared, size_t sz, int fd, void *startaddr) {
    ...
    if (ptr == MAP_FAILED) {
      fprintf(stderr,
              "Couldn't do mmap (%s) : startaddr %p, sz %lx, protInfo=%d, "
              "sharedInfo=%d\n",
